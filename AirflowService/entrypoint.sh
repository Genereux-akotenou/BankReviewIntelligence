...
function create_www_user() {
    ...
    airflow users create \
       ...
       --firstname "BusinessInteligenceReview" \
       --lastname "Team" \
       --email "bri-team@um6p.ma" \
       --role "Admin" \
       --password "${local_password}" || true

    # airflow users create \
    #    --username "${_AIRFLOW_WWW_USER_USERNAME="airflow_user"}" \
    #    --firstname "${_AIRFLOW_WWW_USER_FIRSTNAME="BusinessInteligenceReview Team"}" \
    #    --lastname "${_AIRFLOW_WWW_USER_LASTNAME=""}" \
    #    --email "${_AIRFLOW_WWW_USER_EMAIL="bri-team@um6p.ma"}" \
