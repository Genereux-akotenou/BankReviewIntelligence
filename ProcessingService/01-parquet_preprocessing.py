...
def preprocess_dataframe(df):
    ...
    # Extract topics from Reviewer_Owner_Reply if it's not NaN
    reply_topics = extract_topics(df['Reviewer_Owner_Reply'].dropna())
    df.loc[df['Reviewer_Owner_Reply'].notna(), 'Owner_Reply_Good_Topic'] = reply_topics[:, 0]
    df.loc[df['Reviewer_Owner_Reply'].notna(), 'Owner_Reply_Bad_Topic'] = reply_topics[:, 1]
